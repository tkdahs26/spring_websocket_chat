...
### 카프카를 한 이유
동시 메시지 증가 시 DB 저장을 포함한 동기 처리 구간에서 발생하는 처리 지연을 병목으로 판단하고,웹소켓에서 처리하던 것을 웹소켓에서 수신만하고 Kafka를 도입해 메시지 수신과 후속 처리를 분리했습니다.

# k6테스트 200명의 동시 사용자 총 2,000건의 메시지 부하 테스트
...
평균 지연 시간      272.46 ms 에서 203.39 ms로  25.4% 감소 p95 지연 시간을 487.2ms에서 280ms로 약 42.5% 감소
초당 메시지 처리량  1,030건에서 4,706건으로 약 4.6배 초당 처리량 향상
