...
#### 즉 웹소켓만 쓰면 메세지 한 건씩은 빠르게 처리 하겠지만 많은 건을 동시에 처리하는 할 땐  메시지 수신과 후속 처리를 분리한 kafka를 썼을 시
#### 작업을 분리할수록 유리하므로 kafka가 DB저장시간,초당 메시지 처리량 작업은 높은 격차로 우위에 있다는 것을 알 수 있습니다.
