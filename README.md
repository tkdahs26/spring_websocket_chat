...
### 카프카를 한 이유 
k6와 별도의 db 저장 시간 측정 클래스를 만들고 과부하 테스트 실행.<br>
테스트 후  Kafka를 도입하면 웹소켓의 메세지 처리 지연을 개선하고 특히 DB 저장 작업 속도가 개선 될 수 있다고 판단함.<br>
결국 웹소켓에서만 메세지 작업 처리하던 것을 Kafka를 도입해 웹소켓에서 메시지 수신, 카프카 DB 저장 및 메시지 전송 처리로 분리.<br> 
