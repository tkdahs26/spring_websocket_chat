...
### 카프카를 한 이유 
동시 메시지 입력 시 처리 지연으로 인한 메세지 처리 지연을 개선하기 위해 Kafka 사용.<br>
특히 웹소켓에서 메세지 요청이 몰릴 경우 DB 저장 작업이 병목이 될 수 있다고 판단.<br>
결국 웹소켓에서만 메세지 작업 처리하던 것을 Kafka를 도입해 웹소켓에서 메시지 수신, 카프카 DB 저장 및 메시지 전송 처리로 분리했습니다.<br> 
카프카 도입 후 과부하 테스트 시 DB 저장 작업을 측정하기 위해 별도의 db 저장 지연 시간 측정 클래스를 만들어 평균,p95,최대 등등 저장 시간 측정 실행.<br>
...
###  총 2,000건의 메시지 부하 테스트 5번의 전체 평균 값 
```text 
웹소켓
